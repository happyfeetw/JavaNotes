...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * 树化临界值。
     * 桶的初始实现形式为list
     * 超过该值则使用tree结构替换list结构来实现桶 ---> 转化为红黑树的临界值
     * 当向哈希桶(此时hashmap的基础结构为数组，每个数组的元素位是一个桶)插入一个新元素时，
     * 如果单个桶内元素个数超过了该值，则会将该桶的实现由普通的链表转化为红黑树
     */
     ...
     * 只有表长度大于该值才会进行树化。
     * 防止在哈希表形成之初，桶位充足但单一桶内元素过多而导致整表转化为红黑树。
     */
     ...
     */
    /**
     * 基本的哈希桶节点，哈希表的元素对象，存放哈希表中的键值对(entry)
     * 其子类有:
     * 1. TreeNode, 用于描述红黑树(树化后)的节点
     * 2. 在LinkedHashMap中的Entry子类，用于描述基于双向链表和hashmap实现的Map(linkedHashmap)的节点
     *
     * @param <K>
     * @param <V>
     */
     ...
     */
    /**
     * 计算即将插入map中的键值对的key的哈希值，并将该值的高(16)位扩散(通过异或运算)到低(16)位。
     * 由于哈希表使用2的幂做位掩码，如果不同哈希码仅在当前位掩码的不同位上有区别，则它们会经常发生碰撞(哈希碰撞)
     *
     * @param key
     * @return
     */
    static final int hash(Object key) {
        int h;
        // 无符号右移，将哈希值从高16位扩散到低16位
        // 然后与原哈希码异或操作，目的是充分利用哈希码的32个位，
        // 使经过处理的哈希码在32的分布上更为均匀
        // 由于哈希表默认的初始化大小是1<<4=2^4=16，即一般而言，只用4个位（bit）就可以表示这个长度
        // 所以哈希桶的索引值计算也只用到4个位
        // 而32位机中，哈希函数计算出的哈希码是32位的
    ...
    /**
     * 作用1：初始化哈希表
     * 作用2：对哈希表扩容，大小为原来的一倍，扩容后为原先的两倍大小
     *
     * @return
     */
    final Node<K, V>[] resize() {
        Node<K, V>[] oldTab = table;
        int oldCap = (oldTab == null) ? 0 : oldTab.length;  // 哈希表的原始长度
        int oldThr = threshold; //旧的触发resize的值，哈希表的初始长度与负载因子的乘积
        // 初始化哈希表
        int newCap, newThr = 0;
        if (oldCap > 0) {
            // 哈希表初始长度超过允许的最大长度，设置触发resize的值为整数的最大值后，返回原表
        ...
        table = newTab;
        // 如果oldTab为空，表示原数组没有经历过初始化，则直接返回newTab。
        // 若oldTabl不为空，则表示原数组已被初始化过，需要重新计算元素在扩容后的新数组中的位置，并重新排序，使新顺序与原顺序相同
        if (oldTab != null) {
            // 遍历旧数组的所有下标
            for (int j = 0; j < oldCap; ++j) {
                Node<K, V> e;
                // 旧数组的元素赋值给临时节点e，并把原节点置空，解除引用
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    // 该桶内的链表上只有一个节点
                    if (e.next == null)
                    ...
                    else { // preserve order // 若e是链表的头节点，则对链表中的元素进行重排序，使其均匀分散
                        // 如果桶内是普通链表实现
                        // 新建两个新链表，lo链表和hi链表
                        ...
                        do {
                            next = e.next;
                            // 如果符合条件，说明元素在新表中的索引值与其在旧表中的索引值相同（位置不变）
                            // 然后放入lo链表的尾部
                            if ((e.hash & oldCap) == 0) {
                                if (loTail == null)
                                    // lo链表尾部节点不存在，说明该链表为空
                                    ...
                                    // lo链表存在尾部节点，则将该节点指向尾节点的下一个节点
                                    loTail.next = e;
                                // 普通的情况，直接尾插法
                                loTail = e;
                            } else { //不符合(e.hash & oldCap) == 0的条件，则放入hi链表中的不同位置
                                // 依然是尾插法
                                if (hiTail == null)
                                    hiHead = e;
                                else
                        ...
                        if (hiTail != null) {
                            hiTail.next = null;
                            // 将hi链表的头节点放到新哈希表的（原始表长+当前索引值）的位置上去
                         ...
                         * 所以扩容后，旧元素的新位置（桶位）只有两种可能性：
                         *      1.位置不变，还是在索引值为[j]的位置 -> (e.hash & oldCap) == 0
    ...
    final Node<K, V> removeNode(int hash, Object key, Object value,
        ...
        // 首先判断哈希表是否存在
        if ((tab = table) != null && (n = tab.length) > 0 &&
                (p = tab[index = (n - 1) & hash]) != null) {    // 将哈希表中指定位置的元素赋值给临时节点p
            Node<K, V> node = null; // 桶
            Node<K, V> e; // 创建新的临时节点e
            K k; // 节点中key的范型
            V v; // 节点中value的范型
            if (p.hash == hash &&
                    ((k = p.key) == key || (key != null && key.equals(k))))
                // 如果临时节点p的哈希值与将要被移除的节点的哈希值相同，则将p指向另一个节点对象node
                // 在后面的if块中进行删除操作
                node = p;
            else if ((e = p.next) != null) {
                // 如果临时节点不是尾部节点，将临时节点指向e
                if (p instanceof TreeNode)
                    // p是红黑树节点，则调用getTreeNode获取红黑树节点对象，将其指向临时节点node
                    ...
                    do {    // 遍历链表
                        if (e.hash == hash &&
                            ...
                            // 找到要删除的节点，将该节点指向临时节点node
                            node = e;
                            break;
                        }
                        // 若没有与要删除的节点匹配的节点，则将该节点指向临时节点p
                        p = e;
                    } while ((e = e.next) != null);
                }
            }
